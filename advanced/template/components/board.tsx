...
import { icon_map } from "./svgIcon/icons";
import { TemplateModal, TemplateModalRef } from "./template";
...
const getQuery = async () => {
  ...
  const queryString = Promise.resolve(getInfiWebsdkQuery(devQuerySample));
  return queryString;
};

export function TemplateBoard() {
  const boardDomRef = React.useRef<HTMLDivElement>(null);
  const templateRef = React.useRef<TemplateModalRef>(null);
  const sdkIns = useRef<InfiWebsdkInstanceType>();

  useMemo(() => {
    SvgManager.load(icon_map);
  }, []);

  const getTemplate: GetSdkInstanceConfigs["getTemplate"] = () => {
  ...
  const init = async () => {
    // 获取画布容器 dom
    const container = boardDomRef.current as HTMLDivElement;
    // 初始化画布实例
    const initRes = await InfiWebSdk.getSdkInstance({
       ...
       */
      userInfo: {
        loginName: devQuerySample.loginName,
        userName: "User 0",
      },
      /** 给入画布即将挂在的 DOM 节点 */
  ...
  return (
    <>
      ...
      <TemplateModal
        ref={templateRef}
        getTemplateList={() => {
          return (sdkIns.current as InfiWebsdkInstanceType)?.getTemplateList();
        }}
      />
    </>
  );
